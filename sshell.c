...
// Forks to a child process which handles the command and in the case of redirection
// uses dup() to redirect the output and input appropriately depending on which flag is set
void commandHandler(char *args[], char input[], int backgroundFlag){
    ...
    int pid = fork();
    if(redirectInFlag==1){
        fd = open(args[inRedirectLocation], O_RDONLY,0);
    }
    if(redirectOutFlag==1){
    ...
    if (pid == 0){
        ...
        if(redirectInFlag == 1){
            if(fd < 0){
                // If the file descriptor is bad, prompt error
                errorMessages(ERR_CNTOPEN_INPTFILE);
                exit(0);
            }
            else{
                // Redirect the input to the command
                dup2(fd, 0);
                close(fd);
            ...
            if(fd < 0){
                ...
                // Redirect the output to the specified location
                dup2(fd, 1);
                close(fd);
